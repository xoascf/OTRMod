...
using Bitmap = IronSoftware.Drawing.AnyBitmap;
...
/* TODO: Write texture info in the file name or in the manifest.json */
static void ConvertToPng() {
	string inputDir = ReadPath("Path with textures", checkIfExists: false);
	List<string> dirList =
	...
	foreach (string dir in dirList) {
		...
		foreach (string file in files) {
			...
			OTRMod.Z.Resource? res = OTRMod.Z.Resource.Analyze(resData);
			if (res == null || res.Type != OTRMod.ID.ResourceType.Texture)
				continue;

			OTRMod.Z.Resource.SetData(ref res, resData);
			OTRMod.Z.Texture tex = OTRMod.Z.Texture.LoadFrom(res);

#if NET5_0_OR_GREATER
			tex.GetBitmap().SaveAs(file + ".png");
#else
			tex.GetBitmap().Save(file + ".png", ImageFormat.Png);
#endif
...
/* TODO: Load the texture info from the file name or from the manifest.json (generated by Retro) */
static void ConvertFromPng() {
	string inputDir = ReadPath("Path with PNGs", checkIfExists: false);
	List<string> dirList =
		new(Directory.GetDirectories(inputDir, "*", SearchOption.AllDirectories))
		{ inputDir };
	...
	foreach (string dir in dirList) {
		...
		foreach (string file in files) {
			...
			OTRMod.Z.Texture texture = new(OTRMod.ID.Texture.Codec.RGBA32, sourceImage.Width, sourceImage.Height, imageData, 1, 4f, 4f) {
				IsModded = true
			};

			string outputFileName = Path.GetFileNameWithoutExtension(file);
			string outputPath = Path.Combine(Path.GetDirectoryName(file)!, outputFileName);
			File.WriteAllBytes(outputPath, texture.Formatted());
		}
	}
}

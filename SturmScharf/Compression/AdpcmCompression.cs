...
public static class AdpcmCompression {
	...
	{
		-1, 0, -1, 4, -1, 2, -1, 6, -1, 1, -1, 5, -1, 3, -1, 7, -1, 1, -1, 5, -1, 3, -1, 7, -1, 2, -1, 4, -1, 6,
		-1, 8,
	};
	...
	public static byte[] Decompress(Stream data, int channelCount) {
		...
		BinaryReader reader = new(data ?? throw new ArgumentNullException(nameof(data)));
		using MemoryStream outputstream = new();
		using BinaryWriter writer = new(outputstream);

		reader.ReadByte();
		byte shift = reader.ReadByte();

		for (int i = 0; i < channelCount; i++) {
			short temp = reader.ReadInt16();
			array2[i] = temp;
			writer.Write(temp);
		}
